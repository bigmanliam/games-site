<script>
  ...
  function getShareText() {
    return `Sportle #${gameNumber} ${won ? guesses.length : 'X'}/${MAX_CLUES}\n${getEmojiGrid()}\ndaily-le.us/sportle`
  }
</script>

<svelte:head><title>Sportle — daily-le.us</title></svelte:head>
...
<main class="min-h-screen py-10 px-4">
  <div class="max-w-lg mx-auto">
    ...
    <div class="text-center mb-6">
      ...
      <p class="text-sm text-slate-400">Name the sport from its description. Fewer clues = better.</p>
    </div>
    ...
    {#if !gameOver}
      <div class="flex gap-2">
        <input
          class="input-field"
          placeholder="Name the sport…"
          bind:value={input}
          on:keydown={handleKey}
          autofocus
        />
        <button class="btn text-white shrink-0" style="background:#f97316" on:click={submitGuess} disabled={!input.trim()}>
          Guess
        </button>
      </div>
    ...
    {:else if !won}
      <div class="clue-card result-wrong text-center py-4">
        <p class="text-sm text-slate-300">The sport was <span class="font-bold text-white">{puzzle.answer}</span></p>
      </div>
      <button class="btn w-full mt-3 text-white" style="background:#f97316" on:click={() => showModal = true}>See Results</button>
    {:else}
      <button class="btn w-full text-white" style="background:#f97316" on:click={() => showModal = true}>Share Results</button>
    {/if}
