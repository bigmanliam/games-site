<script>
  ...
  function getShareText() {
    ...
    return `Whodunit #${gameNumber} ${won ? guesses.length : 'X'}/${MAX_CLUES}\n${grid}\ndaily-le.us/whodunit`
  }
</script>

<svelte:head><title>Whodunit — daily-le.us</title></svelte:head>
  ...
  <div class="max-w-lg mx-auto">
    ...
    {:else if !won}
      <div class="clue-card result-wrong text-center py-4">
        <p class="text-sm text-slate-300">The answer was <span class="font-bold text-white">{puzzle.answer}</span></p>
      </div>
      <button class="btn w-full mt-3 text-white" style="background:#a855f7" on:click={() => showModal = true}>See Results</button>
    {:else}
      <button class="btn w-full text-white" style="background:#a855f7" on:click={() => showModal = true}>Share Results</button>
    {/if}
